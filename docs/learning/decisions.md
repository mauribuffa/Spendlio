...
- **Deferred / noted:** the pre-existing hardcoded `×100` in the other manual-entry actions (`createTransactionAction`/`createExpenseAction`/`settleUpAction`) is wrong for non-2-decimal currencies — separate follow-up. AI-suggested category pre-fill (manual pick for v1). A per-transaction detail route (link to the `/transactions` list for now).
