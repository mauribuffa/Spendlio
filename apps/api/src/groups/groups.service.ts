...
export class GroupsService {
  ...
  async list(userId: string): Promise<Group[]> {
    ...
    const groupIds = rows.map((g: { id: string }) => g.id);
    ...
    for (const m of members as { groupId: string; personId: string }[]) {
    ...
    return rows.map(
      (g: { id: string; userId: string; name: string; createdAt: Date; updatedAt: Date }): Group => ({
        id: g.id,
        userId: g.userId,
        name: g.name,
